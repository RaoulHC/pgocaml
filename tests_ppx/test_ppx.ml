...
let _employee_exists dbh ?email n =
  [%pgsql
    dbh
      "SELECT EXISTS (SELECT 1 FROM employees WHERE name = $n AND email = \
       $?email)"]
...
let () =
  ...
  let insert name pay email =
    [%pgsql
      dbh
        "insert into employees (name, salary, email) values ($name, $pay, \
         $?email)"]
  in
  ...
  let rows =
    [%pgsql
      dbh "load_custom_from=tests_ppx/config.sexp"
  ...
  in
  List.iter (fun obj -> print_endline obj#pp) rows;
  let uid = Userid.from_string "69" in
  let salary = "$420.00" in
  let () =
    [%pgsql
      dbh "load_custom_from=tests_ppx/config.sexp"
        "INSERT INTO customtable (userid, salary) VALUES (${uid:userid}, \
         $salary)"]
  in
  let rows' =
    [%pgsql.object
      dbh "load_custom_from=tests_ppx/config.sexp" "show"
