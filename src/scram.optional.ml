...
let client_first_message ?(user = "") mechanisms =
  if List.mem scram_sha_256_mechanism mechanisms then
    ...
    Error
      (Format.sprintf "PGOCaml: None of %s mechanism is implemented"
         (String.concat ", " mechanisms))

let xor a b =
  if String.length b <> String.length a then
    invalid_arg "xor: both strings must be of same lengths";
...
let hi ~password:key ~salt ~count =
  ...
  let rec aux acc x = function
    ...
    | i ->
        ...
        aux acc (xor x acc) (i - 1)
  in
  let start = hmac ~key (salt ^ "\000\000\000\001") in
  aux start start count

let proof ~user ~password ~client_nonce ~server_nonce ~salt64 ~gs2_header64
    ~count =
  ...
  let auth_message =
    Format.sprintf "n=%s,r=%s,r=%s,s=%s,i=%d,c=%s,r=%s" user client_nonce
      server_nonce salt64 count gs2_header64 server_nonce
  in
...
let client_final_message ?(user = "") ~password ~client_info s =
  ...
  in
  let err =
    Error "PGOCaml: server SASL authentication message not understood"
  in
  ...
  | Some server_nonce, Some salt64, Some count ->
      if String.starts_with ~prefix:client_nonce server_nonce then
        ...
        let proof, signature =
          proof ~user ~password ~client_nonce ~server_nonce ~salt64
            ~gs2_header64 ~count
        in
        let s =
          Format.sprintf "c=%s,r=%s,p=%s" gs2_header64 server_nonce proof
        in
        Ok (s, signature)
      else err
  | _ -> err
