...
module type PGOCAML_GENERIC = sig
  ...
  (** For errors generated by the PostgreSQL database back-end. The first
      argument is a printable error message. The second argument is the complete
      set of error fields returned from the back-end.
  ...
  *)
  ...
  (** Close the database handle.
      ...
      You must call this after you have finished with the handle, or else you
      will get leaked file descriptors. *)
  ...
  *)
  ...
  (** Maximum message length accepted from the back-end.
      ...
      run out of memory (particularly on 64 bit machines), causing a possible
      denial of service.
  ...
  (** Verbosity.
      ...
      0 means don't print anything. 1 means print short error messages as
      returned from the back-end. 2 means print all messages as returned from
      the back-end.
  ...
  (** Attach some private data to the database handle.
      ...
      means that in most programs you will not be able to attach private data to
      the database handle. *)

  val private_data : 'a t -> 'a
  (** Retrieve some private data previously attached to the database handle. If
      no data has been attached, raises [Not_found].
  ...
  (** [prepare conn ~query ?name ?types ()] prepares the statement [query] and
      optionally names it [name] and sets the parameter types to [types].

      If no name is given, then the "unnamed" statement is overwritten. If no
  ...
  (** [execute conn ?name ~params ()] executes the named or unnamed statement
      [name], with the given parameters [params], returning the result rows (if
      any).
      ...
      The optional [?portal] parameter may be used to name the portal created in
      step (1) above (otherwise the unnamed portal is used). This is only
      important if you want to call {!describe_portal} to find out the result
      types. *)
  ...
  *)
  ...
  (** [describe_portal conn ?portal ()] describes the named or unnamed portal's
      result types. *)
  ...
  (** Returns the OCaml equivalent type name to the PostgreSQL type [oid].
      ...
      the OID for PostgreSQL's internal [int4] type is [23].
  ...
  *)
  ...
  (** The following conversion functions are used by pa_pgsql to convert values
