...
module type PGOCAML_GENERIC = sig
  ...
  (** For errors generated by the PostgreSQL database back-end. The * first
      argument is a printable error message. The second argument * is the
      complete set of error fields returned from the back-end. * See
  ...
  (** Close the database handle. You must call this after you have * finished
      with the handle, or else you will get leaked file * descriptors. *)
  ...
  val transact :
    ...
    ('a t -> 'b monad) ->
    'b monad
  (** [transact db ?isolation ?access ?deferrable f] wraps your * function [f]
      inside a transactional block. * First it calls [begin_work] with
      [isolation], [access] and [deferrable], * then calls [f] and do [rollback]
      if [f] raises * an exception, [commit] otherwise. *)

  (** {6 Serial column} *)

  val serial : 'a t -> string -> int64 monad
  (** This is a shorthand for [SELECT CURRVAL(serial)]. For a table * called
      [table] with serial column [id] you would typically * call this as
      [serial dbh "table_id_seq"] after the previous INSERT * operation to get
  ...
  (** Maximum message length accepted from the back-end. The default * is
      [Sys.max_string_length], which means that we will try to read as * much
      data from the back-end as we can, and this may cause us to * run out of
      memory (particularly on 64 bit machines), causing a * possible denial of
      service. You may want to set this to a smaller * size to avoid this
      happening. *)

  val verbose : int ref
  (** Verbosity. 0 means don't print anything. 1 means print short * error
      messages as returned from the back-end. 2 means print all * messages as
      returned from the back-end. Messages are printed on [stderr]. * Default
      verbosity level is 1. *)

  val set_private_data : 'a t -> 'a -> unit
  (** Attach some private data to the database handle. * * NB. The pa_pgsql
      camlp4 extension uses this for its own purposes, which * means that in
      most programs you will not be able to attach private data * to the
      database handle. *)

  val private_data : 'a t -> 'a
  (** Retrieve some private data previously attached to the database handle. *
      If no data has been attached, raises [Not_found]. * * NB. The pa_pgsql
      camlp4 extension uses this for its own purposes, which * means that in
      most programs you will not be able to attach private data * to the
      database handle. *)
  ...
  val prepare :
    ...
    unit ->
    unit monad
  (** [prepare conn ~query ?name ?types ()] prepares the statement [query] * and
      optionally names it [name] and sets the parameter types to [types]. * If
      no name is given, then the "unnamed" statement is overwritten. If * no
      types are given, then the PostgreSQL engine infers types. * Synchronously
      checks for errors. *)
  ...
  val execute :
    ...
    unit ->
    row list monad
  (** [execute conn ?name ~params ()] executes the named or unnamed * statement
      [name], with the given parameters [params], * returning the result rows
      (if any). * * There are several steps involved at the protocol layer: *
      (1) a "portal" is created from the statement, binding the * parameters in
      the statement (Bind). * (2) the portal is executed (Execute). * (3) we
      synchronise the connection (Sync). * * The optional [?portal] parameter
      may be used to name the portal * created in step (1) above (otherwise the
      unnamed portal is used). * This is only important if you want to call
      {!PGOCaml.describe_portal} * to find out the result types. *)
  ...
  (** [close_statement conn ?name ()] closes a prepared statement and frees * up
      any resources. *)
  ...
  *)
  ...
  (** [inject conn ?name query] executes the statement [query] * and optionally
      names it [name] and gives the result. *)

  val alter : 'a t -> ?name:string -> string -> unit monad
  (** [alter conn ?name query] executes the statement [query] * and optionally
      names it [name]. Same as inject but ignoring the result. *)
  ...
  (** Returns the OCaml equivalent type name to the PostgreSQL type [oid]. * For
      instance, [name_of_type (Int32.of_int 23)] returns ["int32"] because * the
      OID for PostgreSQL's internal [int4] type is [23]. As another * example,
...
module Make (Thread : THREAD) = struct
  ...
  let receive_raw_message { ichan; chan; _ } =
    ...
    if len > !max_message_length then
      ...
      in
      loop len >>= fun () ->
      fail (Error "PGOCaml: back-end message is longer than max_message_length")
    else
  ...
  let string_of_msg_t = function
    ...
    | AuthenticationSASL mechanisms ->
        sprintf "AuthenticationSASL %S" (String.concat "," mechanisms)
    | AuthenticationSASLContinue str ->
        sprintf "AuthenticationSASLContinue %S" str
    | AuthenticationSASLFinal str -> sprintf "AuthenticationSASLFinal %S" str
    | BackendKeyData (i1, i2) -> sprintf "BackendKeyData %ld, %ld" i1 i2
    ...
    | ParameterDescription fields ->
        sprintf "ParameterDescription [%s]"
          (String.concat "; " (List.map (fun oid -> sprintf "%ld" oid) fields))
    | ParameterStatus (s1, s2) -> sprintf "ParameterStatus %S, %S" s1 s2
    ...
    | RowDescription fields ->
        sprintf "RowDescription [%s]"
          (String.concat "; "
             (List.map
                (fun (name, table, col, oid, len, modifier, format) ->
                  sprintf "%s %ld %d %ld %d %ld %d" name table col oid len
                    modifier format)
                fields))
    | UnknownMessage (typ, msg) -> sprintf "UnknownMessage %c, %S" typ msg
  ...
  let parse_backend_message (typ, msg) =
    ...
    let get_char where =
      if !pos < len then (
        ...
        raise
          (Error ("PGOCaml: parse_backend_message: " ^ where ^ ": short message"))
    in
    ...
    let msg =
      ...
      | 'T' ->
          ...
          for _ = 0 to nr_fields - 1 do
            ...
            fields :=
              (name, table, column, oid, length, modifier, format) :: !fields
          done;
  ...
  let rec receive_message conn =
    ...
    | ParameterStatus _ | NoticeResponse _ | NotificationResponse ->
  ...
  let print_ErrorResponse fields =
    (if !verbose >= 1 then
       try
         let severity =
           try Some (get_severity fields) with Not_found -> None
         in
         let severity_string =
           match severity with Some s -> show_severity s | None -> "UNKNOWN"
         in
  ...
  let pg_error ?conn fields =
    ...
    let str =
      try
        let severity_string =
          try show_severity @@ get_severity fields with Not_found -> "UNKNOWN"
        in
        let code = List.assoc 'C' fields in
        let message = List.assoc 'M' fields in
  ...
  let profile_op uuid op detail f =
    let chan =
      try
        ...
        Some chan
      with Not_found | Sys_error _ -> None
    in
    match chan with
    ...
    | Some chan -> (
        ...
        let row =
          [
            ...
            (match ret with Ret _ -> "ok" | Exn exn -> Printexc.to_string exn);
          ]
          @ detail
        in
  ...
  let describe_connection ?host ?port ?user ?password ?database
      ...
      | None -> (
          ...
          with Not_found -> (
            try
              ...
              pw.Unix.pw_name
            with Not_found -> PGOCaml_config.default_user))
    in
    ...
    let password =
      ...
      | None -> (
          ...
          with Not_found -> PGOCaml_config.default_password)
    in
    ...
    let host =
      let host_or_socket s =
        if String.length s > 0 && s.[0] = '/' then `Unix_domain_socket_dir s
        else `Hostname s
      in
      ...
      | Some _, Some _ ->
          raise
            (Failure
               "describe_connection: it's invalid to specify both a HOST and a \
                unix domain socket directory")
      | Some s, None -> host_or_socket s
      | None, Some s -> `Unix_domain_socket_dir s
      | None, None -> (
          try host_or_socket (Sys.getenv "PGHOST")
          with Not_found ->
  ...
  let connection_desc_to_string key =
    Printf.sprintf "host=%s, port=%s, user=%s, password=%s, database=%s"
      (match key.host with
      | `Unix_domain_socket_dir _ -> "unix"
      | `Hostname s -> s)
      (string_of_int key.port) key.user
  ...
  let connect ?host ?port ?user ?password ?database ?unix_domain_socket_dir
    ...
    let { user; host; port; database; password } =
      match desc with
      | None ->
          describe_connection ?host ?port ?user ?password ?database
            ?unix_domain_socket_dir ()
      | Some desc -> desc
    in
    ...
    let sockaddrs =
      ...
      | `Hostname hostname ->
          let addrs =
            Unix.getaddrinfo hostname (sprintf "%d" port)
              [ Unix.AI_SOCKTYPE Unix.SOCK_STREAM ]
          in
          if addrs = [] then
            raise (Error ("PGOCaml: unknown host: " ^ hostname))
          else List.map (fun { Unix.ai_addr = sockaddr; _ } -> sockaddr) addrs
    ...
    let uuid =
       ...
       *)
      let ppid = try Unix.getppid () with Invalid_argument _ -> 0 in
      sprintf "%s %d %d %g %s %g" (Unix.gethostname ()) (Unix.getpid ()) ppid
        (Unix.gettimeofday ()) Sys.executable_name
    ...
    let do_connect () =
      sock_channels >>= fun (ichan, chan) ->
        ...
        (fun () ->
          ...
          let conn = { ichan; chan; private_data = None; uuid } in
          ...
          let rec loop ?sasl msg =
            ...
            | AuthenticationCryptPassword _salt ->
                ...
                fail
                  (Error
                     "PGOCaml: crypt password is deprecated since version 8.4")
            | AuthenticationMD5Password salt ->
                let password =
                  "md5"
                  ^ Digest.to_hex
                      (Digest.string
                         (Digest.to_hex (Digest.string (password ^ user)) ^ salt))
                in
                let msg = new_message 'p' in
                (* PasswordMessage *)
                add_string msg password;
                loop ?sasl (Some msg)
            | AuthenticationSASL mechanisms -> (
                match Scram.client_first_message mechanisms with
            ...
            | AuthenticationSASLContinue str -> (
                ...
                | Some (`client_info client_info) -> (
                    match
                      Scram.client_final_message ~client_info ~password str
                    with
                    | Error e -> fail (Error e)
                    | Ok (s, signature) ->
                        let msg = new_message 'p' in
  ...
  let close conn =
    let do_close () =
        ...
        (fun () ->
          ...
          flush conn.chan >>= fun () -> return None)
        (fun e -> return (Some e))
      >>= fun e ->
      (* Closes the underlying socket too. *)
      close_in conn.ichan >>= fun () ->
      match e with None -> return () | Some e -> fail e
    in
    ...
    let do_prepare () =
      ...
      let rec loop () =
        ...
        | _ ->
            fail
              (Error
                 ("PGOCaml: unknown response from parse: " ^ string_of_msg_t msg))
      in
      loop ()
    in
  ...
  let iter_execute conn name portal params proc () =
    ...
    let rec loop () =
      ...
      | _ ->
          fail
            (Error ("PGOCaml: unknown response message: " ^ string_of_msg_t msg))
    in
    loop ()
  ...
  let do_execute conn name portal params rev () =
    ...
    iter_execute conn name portal params
      (fun fields -> return (rows := fields :: !rows))
      ()
    >>= fun () ->
  ...
  let transact conn ?isolation ?access ?deferrable f =
    begin_work ?isolation ?access ?deferrable conn >>= fun () ->
      ...
      (fun () ->
        f conn >>= fun r ->
        commit conn >>= fun () -> return r)
      (fun e -> rollback conn >>= fun () -> fail e)
  ...
  let close_statement conn ?(name = "") () =
    ...
    let rec loop () =
      ...
      | _ ->
          fail
            (Error
               ("PGOCaml: unknown response from close: " ^ string_of_msg_t msg))
    in
    loop ()
  ...
  let inject db ?name query =
    ...
    close_statement db ?name () >>= fun () -> return ret
  ...
  let expect_rfq conn ret =
    ...
    | msg ->
        fail
        @@ Error
             ("PGOCaml: unknown response from describe: " ^ string_of_msg_t msg)
  ...
  let describe_statement conn ?(name = "") () =
    ...
    | ParameterDescription params ->
        let params = List.map (fun oid -> { param_type = oid }) params in
        return params
    | _ ->
        fail
          (Error
  ...
  let name_of_type = function
    ...
    | 1021_l | 1022_l -> "float_array" (* FLOAT4[], FLOAT8[] *)
  ...
  let string_of_inet (addr, mask) =
    ...
    if mask = hostmask then addr
    else if mask >= 0 && mask < hostmask then addr ^ "/" ^ string_of_int mask
  ...
  let string_of_timestamptz (cal, tz) =
    ...
    | Time_Zone.Local ->
        ...
        if gap >= 0 then sprintf "+%02d" gap else sprintf "-%02d" (-gap)
  ...
  let string_of_int16_array a =
    string_of_any_array (List.map (option_map Stdlib.string_of_int) a)
  ...
  let comment_src_loc () =
    ...
    | Some x -> (
        ...
        | _ ->
            failwith
              (Printf.sprintf "Unrecognized option for 'PGCOMMENT_SRC_LOC': %s"
                 x))
    | None -> PGOCaml_config.default_comment_src_loc

  open Sexplib.Std

  type custom_rule_payload = { serialize : string; deserialize : string }
  [@@deriving sexp]
  ...
  let find_custom_typconvs =
    let open Rresult in
    let loadconvs fname =
      try Ok (Sexplib.Sexp.load_sexp_conv_exn fname custom_rules_conf_of_sexp)
      with exn ->
        let cwd = Unix.getcwd () in
        Error
          (Printf.sprintf "Error parsing custom typeconvs file in %s: %s" cwd
    ...
    fun ?typnam ?lookin ?colnam ?argnam () ->
      ...
      | Some x -> (
          ...
          match convs with Ok x -> Ok x | Error e -> Error e
          (*>>= fun blep ->
  ...
  let string_of_bytea b =
    let (`Hex b_hex) = Hex.of_string b in
    "\\x" ^ b_hex
  ...
  let inet_of_string =
    ...
    fun str ->
      ...
      (* optional match *)
      if mask = "" then (addr, if Re.Group.get subs 2 = "." then 32 else 128)
  ...
  let point_of_string =
    ...
    fun str ->
      ...
      with _ -> failwith "point_of_string"
  ...
  let time_of_string str =
    ...
    let str = if n > 8 && str.[8] = '.' then String.sub str 0 8 else str in
    Printer.Time.from_string str

  let timestamp_of_string str =
    (* Remove trailing ".microsecs" if present. *)
    let n = String.length str in
    let str = if n > 19 && str.[19] = '.' then String.sub str 0 19 else str in
  ...
  let timestamptz_of_string str =
    ...
    let tz =
      ...
      | Some tz ->
          let sgn =
            match tz.[0] with '+' -> 1 | '-' -> -1 | _ -> assert false
          in
  ...
  let re_interval =
    ...
    let time_period unit_name =
      [ group (rep1 digit); space; str unit_name; opt (char 's') ] |> seq |> opt
    in
  ...
  let interval_of_string =
    let int_opt subs i =
      try int_of_string (Re.Group.get subs i) with Not_found -> 0
    in
    fun str ->
      try
        let sub = Re.exec re_interval str in
        Calendar.Period.make (int_opt sub 1) (* year *)
          (int_opt sub 2) (* month *)
          (int_opt sub 3) (* day *)
          (int_opt sub 4) (* hour *)
          (int_opt sub 5) (* min *)
          (int_opt sub 6)
        (* sec *)
      with Not_found -> failwith ("interval_of_string: bad interval: " ^ str)
    ...
    let add_field accum =
      ...
      let field =
        if x = "NULL" then None
          ...
          if n >= 2 && x.[0] = '"' then Some (String.sub x 1 (n - 2))
          else Some x
      in
      field :: accum
    in
  ...
  let int16_array_of_string str =
    List.map (option_map Stdlib.int_of_string) (any_array_of_string str)
...
*)
  let bytea_of_string str =
    if String.starts_with str "\\x" then bytea_of_string_hex str
