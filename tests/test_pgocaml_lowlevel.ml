...
let print_row i row =
  printf "row %d: [%s]\n" i
    (String.concat "; "
       (List.map (function None -> "NULL" | Some str -> sprintf "%S" str) row))
...
let () =
  ...
  let i = ref 0 in
  PGOCaml.cursor dbh ~name ~params:[] (fun row ->
      incr i;
      print_row !i row);
  ...
  let types = [ 23l; 23l ] in
  (* 23 = int4 *)
  let name = "sum_query" in
  ignore (PGOCaml.prepare dbh ~query ~name ~types ());
  let i = ref 0 in
  PGOCaml.cursor dbh ~name ~params:[ Some "1"; Some "2" ] (fun row ->
      incr i;
      print_row !i row);
  ...
  let query =
    "insert into employees (name, salary, email) values ($1, $2, $3)"
  in
  ignore (PGOCaml.prepare dbh ~query ());
  ...
  ignore (PGOCaml.execute dbh ~params:[ Some "Ann"; Some "10000.00"; None ] ());
  ignore (PGOCaml.execute dbh ~params:[ Some "Bob"; Some "45000.00"; None ] ());
  ignore (PGOCaml.execute dbh ~params:[ Some "Jim"; Some "20000.00"; None ] ());
  ignore (PGOCaml.execute dbh ~params:[ Some "Mary"; Some "30000.00"; None ] ());
