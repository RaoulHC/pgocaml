...
let point_of_string =
  ...
  let tests =
    ...
    |> List.map (fun (s, pt) ->
           s >:: fun () -> assert_equal ~printer pt (PGOCaml.point_of_string s))
  in
  ("nan" >:: fun () -> ignore (PGOCaml.point_of_string "(nan ,  NaN )"))
  :: tests
...
let interval_of_string =
  ...
  |> List.map (fun (s, p) ->
         s >:: fun () -> assert_equal ~printer p (PGOCaml.interval_of_string s))
