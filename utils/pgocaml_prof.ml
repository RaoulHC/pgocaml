      ...
      try
        ...
        let params, prog =
          ...
          | _ ->
              ignore
                (sprintf
                   "connection %s did not start with a 'connect' operation" uuid)
        in

        set_connection params (fun data ->
            { data with progs = prog :: List.filter (( <> ) prog) data.progs });
        ...
        List.iter
          ...
          rows
      with Ignore msg -> eprintf "warning: %s\n" msg)
    files

(* Clean up temporary directory. *)
let () =
  List.iter (fun filename -> Unix.unlink (tmpdir // filename)) files;
  Unix.rmdir tmpdir
...
let queries = Hashtbl.fold (fun query data xs -> (query, data) :: xs) queries []
...
let queries =
  let f (_, { prep_time = prep_time1; exec_time = exec_time1 })
